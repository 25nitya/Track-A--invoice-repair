...
def insert_invoice(db, row):
    # Invoice identity is based on (customer_id, invoice_number).
    # Previously, every import directly inserted the invoice, so
    # re-importing the same invoice created duplicate records.
    old = invoice_by_key(db, row['customer_id'], row['invoice_number'])

    if old:
        # If the identity and invoice details are unchanged, this is
        # an idempotent re-import, so skip it instead of creating a duplicate.
        if old['amount'] == row['amount'] and old['due_date'] == row['due_date']:
            return 'skipped'

        # The same invoice identity cannot represent different invoice
        # details. Reject the new row and preserve the existing record.
        raise ValueError('Invoice already exists with different details')

    # No existing invoice has this customer + invoice number,
    # so this is a new invoice and can be inserted.
    db.execute('''INSERT INTO invoices (customer_id, invoice_number, amount, due_date)
                  VALUES (:customer_id, :invoice_number, :amount, :due_date)''', row)
